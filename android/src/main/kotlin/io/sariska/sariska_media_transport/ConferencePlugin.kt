...
class ConferencePlugin: FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler {
    private lateinit var methodChannel: MethodChannel
    private lateinit var eventChannel: EventChannel
    private var eventSink: EventChannel.EventSink? = null
    private val manager = ConferenceManager { action, m -> emit(action, m) }
    private val handler = Handler(Looper.getMainLooper())

    fun initPlugin(binaryMessenger: BinaryMessenger) {
        methodChannel = MethodChannel(binaryMessenger, "conference")
        methodChannel.setMethodCallHandler(this)
        eventChannel = EventChannel(binaryMessenger, "conference/events")
    ...
    private fun emit(action: String, m: Map<String, Any?>?) {
        handler.post {
            val event: MutableMap<String, Any?> = mutableMapOf("action" to action)
            m?.let { event.putAll(it) }
